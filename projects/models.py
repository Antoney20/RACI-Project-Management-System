...
#     - A (Accountable): Signs off, ultimate ownership
...
#     """Sprint/Timeline for grouping projects"""
...
# class ProjectRACIAssignment(models.Model):
#     """
#     RACI Assignment Model
    
#     CRITICAL RULES:
#     1. External users can ONLY be assigned as C (Consulted) or I (Informed)
#     2. Internal staff can be assigned any role (R, A, C, I)
#     3. Each project should have at least ONE Accountable (A) person
#     4. Multiple people can have same role (e.g., multiple Responsible)
#     """
...
#         on_delete=models.SET_NULL,
#         null=True,
#         related_name="assigned_raci_roles"
#     )
#     assigned_at = models.DateTimeField(auto_now_add=True)

#     class Meta:
#         db_table = "projects_raci_assignment"
...
#     def __str__(self):
#         return f"{self.project.name} - {self.user.username} ({self.get_raci_role_display()})"

#     def clean(self):
#         """Validate RACI assignment rules"""

        
#         # Blocked users cannot be assigned
#         if self.user.is_blocked():
#             raise ValidationError(
#                 f"Cannot assign blocked user {self.user.username} to project."
#             )
...
#         upload_to="projects/materials/%Y/%m/",
#         blank=True,
#         null=True
#     )
...
#         return f"{self.project.name} - {self.link_title}"

#     def clean(self):
#         if self.material_type == "file" and not self.file:
#             raise ValidationError("File is required for file type materials")
#         if self.material_type == "link" and not self.link_url:
#             raise ValidationError("URL is required for link type materials")
...
#     def __str__(self):
#         return f"{self.project.name} - {self.user.username} - {self.created_at}"
